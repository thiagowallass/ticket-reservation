...
public class Airplane {
    ...
    public Seat findSeat(String nameButton){
        String seat;
        int row;
        
        row = getRow(nameButton);
        seat = getSeat(nameButton);
        
        if(row < first.rows.length){
            return first.rows[row].seats[Seat.converteToPos(seat)];
        }else if(row < first.rows.length + economic.rows.length){
            row = row - 5;
            return economic.rows[row].seats[Seat.converteToPos(seat)];
        }else{
            return null;
        }
    ...
    public String toString(){
        String back = "";
        back =  this.company + "\t" + this.flight + "\t" + this.departureTime + "\t" + this.destination + "\t" + this.gate;
        return back;
    }
